export const configs = {
  "env": "development",
  "github_api": {
    "user": "user",
    "password": "password"
  },
  "copyright_year": "2021",
  "root_directory": ".",
  "deno": {
    "latest_version": "v1.11.5"
  },
  "deno_std": {
    "latest_version": "0.100.0"
  },
  "dmm": {
    "base_url": "/dmm/v1.x",
    "latest_version": "v1.3.2",
    "latest_url_deno_land": "https://deno.land/x/dmm@v1.3.2/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/dmm@v1.3.2/mod.ts",
    "versions": [
      "v1.x"
    ]
  },
  "drash": {
    "base_url": "/drash/v1.x",
    "latest_version": "v1.5.0",
    "latest_url_deno_land": "https://deno.land/x/drash@v1.5.0/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/deno-drash@v1.5.0/mod.ts",
    "versions": [
      "v1.x"
    ]
  },
  "line": {
    "base_url": "/line/v0.x",
    "latest_version": "v0.1.0",
    "latest_url_deno_land": "https://deno.land/x/line@v0.1.0/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/line@v0.1.0/mod.ts",
    "versions": [
      "v0.x"
    ]
  },
  "rhum": {
    "base_url": "/rhum/v1.x",
    "latest_version": "v1.1.11",
    "latest_url_deno_land": "https://deno.land/x/rhum@v1.1.11/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/rhum@v1.1.11/mod.ts",
    "versions": [
      "v1.x"
    ]
  },
  "wocket": {
    "base_url": "/wocket/v0.x",
    "latest_version": "v0.6.3",
    "latest_url_deno_land": "https://deno.land/x/wocket@v0.6.3/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/wocket@v0.6.3/mod.ts",
    "versions": [
      "v0.x"
    ]
  },
  "sinco": {
    "base_url": "/sinco/v2.x",
    "latest_version": "v2.0.0",
    "versions": [
      "v2.x",
      "v1.x"
    ],
    "latest_url_deno_land": "https://deno.land/x/sinco@v2.0.0/mod.ts",
    "latest_url_nest_land": "https://x.nest.land/sinco@v2.0.0/mod.ts"
  }
};
